...
| OURS | `settle` |
| HYBRID | Routes by decline class: OURS on `auth_abandoned`, B2's fixed ladder on every other class. No new model, no new mechanism, no tuning — it composes two arms that already exist, using the per-class result to decide which. It exists to measure the ceiling of class-based routing, and it is NOT the submitted policy. |
...
- 2026-09-02 — A129: §16 — `GET /` still returns 501 and `viewer/index.html` is opened from the filesystem. The route was wired at CP14 and reverted before commit: `tests/test_webhook.py` asserts the stub returns 501, that file was not on the CP14 allowlist, and a green gate was the stated condition. It lands at CP15 with the voice endpoint, which opens both files anyway. No companion route will serve `out/` — the table stays at exactly three, and a viewer convenience is not a reason to widen it.
- 2026-09-02 — A130: §3 — the viewer's data contract. `settle/eval/report.py` writes `out/viewer_data.json` and every value in it is finished in Python — percentages, rupee strings, labels, counts. The page renders and never computes, which is the same span-locate / code-evaluate split §11 applies to text, applied to the UI. A viewer that derived a rate would be a second implementation of a metric and the two would disagree the first time either changed. Test VIW-2.
- 2026-09-02 — A131: §3 — the page carries an embedded copy of its data as well as reading the JSON file. A browser will not `fetch()` a sibling from a `file://` origin, and the viewer has to open from a cloned repo with nothing running. Served, it prefers the file so a regenerated run shows on reload. Test VIW-4, which executes the page under Node with `fetch` throwing rather than asserting about its markup.
- 2026-09-02 — A132: §5.4 — the case trace is where `Alternative` earns its shape. Every decision shown lists every option the policy priced, sorted by expected value, with `p_success`, `ev_paise`, `legal` and `block_gate`, the chosen row marked and blocked rows in among the rest rather than in a footnote. Traces are capped per case; the alternatives inside a shown decision never are. Tests VIW-3, VIW-5.
- 2026-09-02 — A127: §10.1 — `settle/agent/train.py` writes the timing block of `out/model_report.json` at training time. The figures had lived only in its stdout, which is how the README came to quote pre-A93 values that reproduced nothing. It merges rather than overwrites: `settle/agent/` may not import `settle.sim` (INV-8), so it cannot run an arm and cannot produce the SF-2 decomposition that shares the file, and a training run that clobbered that block would be the agent package deleting evidence it is not entitled to generate. `--model-report` is deliberately independent of `--out`, so a run training into a scratch directory can still refresh the committed report. Recorded at CP13.3, applied at CP15 (F31).
- 2026-09-02 — A128: `.gitignore` negates `out/model_report.json`. It had been committed with `git add -f` at CP13.2 and CP13.3, which is the workaround `out/razorpay_demo.json` needed between CP12 and CP12.1. Recorded at CP13.3, applied at CP15 (F31).
- 2026-09-03 — A133: §16 — `POST /voice/extract` and `GET /` both work. `/voice/extract` takes the audio as the raw request body rather than a multipart form: `python-multipart` is not pinned, and an undeclared dependency would make the route work here and fail for anyone cloning the repo — the same trade CP12 made over `httpx`. `GET /` serves `viewer/index.html`. The table is still exactly three.
- 2026-09-03 — A134: §11 — transcription findings, measured at CP15 on the four clips and recorded because they cost a day to find. (a) With `language="hi"` and no prompt, `gpt-4o-transcribe` returns **Urdu in Arabic script**, not Devanagari. The prior warning was that the language parameter steers recognition and not output script; the reality is a third script no parser was written for. (b) It truncates: clip 1's un-prompted transcript drops the self-correction that is the clip's entire purpose. (c) A romanised-Hinglish `prompt` fixes both. (d) At the default temperature the same audio gave three different strings in four calls, one truncated; `temperature=0` is 4/4 identical. Temperature is a correctness setting here, not a preference. `PROMPT_VERSION` is part of the cache key, so a steering change invalidates the transcripts it produced.
- 2026-09-03 — A135: §11 — `settle/text/promise.py` is a trace layer over `settle/text/classify.py`, not a second implementation. The CP7.0 contract is unchanged: `find_date_spans` locates, `parse_date_span` evaluates, `validate` bounds, `classify_reply` decides. It adds one span kind, `next_month`, which is always rejected — a month with no day is not a commitment — so that clip 1's self-correction can be shown being considered and set aside rather than never looked at. Tests VOI-3, VOI-7.
- 2026-09-03 — A136: §11 — the voice trace is the product, not the verdict. `POST /voice/extract` returns every located span, what each parsed to, which validation rejected and why, the verdict, and the action or its explicit absence. Clip 3 is the case that matters: the honest answer is that nothing happens, and the screen has to make "nothing happened" legible. Test VOI-5.
- 2026-09-03 — A137: §16 — `POST /voice/extract` takes a multipart `audio` field now that `python-multipart` is pinned, and still accepts a raw body when the content type is not multipart. The raw path is kept rather than replaced: the viewer posts that way, and a route that broke its only caller to adopt a tidier signature would be a refactor charged to the demo.
- 2026-09-03 — A138: `.gitignore` audited at CP16 against every path in `git ls-files out/`. Nine committed artefacts, nine negations. `out/razorpay_demo.json` and `out/voice_demo.json` were tracked only because they had been force-added at a checkpoint whose allowlist excluded `.gitignore` — the third occurrence, and the reason the audit happened rather than a fourth one-line fix.
- 2026-09-03 — A139: §11 — `next_month` is located by `settle/text/promise.py` and by nothing else. `settle/text/classify.py`, which the batch runs on, has no equivalent pattern, so the voice lab locates one span the batch would not. The verdict is identical in every observed case because the span is always rejected, and Known Limitations records that "identical because the difference is always discarded" is weaker than "identical".
- 2026-09-04 — A140: §14.1 — HYBRID added to the arms table. Routes by decline class: OURS on `auth_abandoned`, B2's ladder on every other class. It composes two existing arms and adds no model, mechanism or parameter, so the number it produces measures the routing rather than any tuning. ENFORCE only — INV-11 reaches it through what it contains, because it holds OURS. Not the submitted policy. Tests ARM-5, ARM-6, ARM-7.
- 2026-09-04 — A141: §14.1 — the routing key is `classify(case.decline_code)`, a pure function of an `ObservedCase` field. There is no `decline_class` field on `ObservedCase` and the router never reads `CaseState`, so the routing decision is identical at tick 0 and tick 700 and a case belongs to one delegate for its whole life. That is what makes ARM-7's byte-identity possible: a router switching mid-case would produce a trajectory neither source arm ever visits.
- 2026-09-04 — A142: §14.4 — `opt_outs_induced` is reported per arm. It was listed in the headline table since CP0 and nothing produced it; counted now from the ledger's `OPTED_OUT` reason code. It is the cost side of contact volume, and HYBRID is what made its absence conspicuous: 285 opt-outs against OURS's 1.
- 2026-09-04 — A143: §14.1 — the HYBRID measurement, recorded because it answers a question a reader will ask. At 10,000 cases HYBRID recovers 31.87% against OURS's 28.37%, +3.50 points and ₹227,318. It costs 9,192 contacts against 36 — 255x — 285 opt-outs against 1, and 2.34x the cost per ₹100 recovered. Its per-class rates equal B2's on every ladder-routed class and OURS's on `auth_abandoned`, exactly, which is the composition being visible in the output rather than asserted. The restraint result does not survive routing: HYBRID's contact volume is 66% of the fixed ladder's.
- 2026-09-04 — A144: §14.1 — HYBRID is marked on chart 1 the way B3 is: its own colour, a diamond marker, and a COMPOSED label naming what it is made of. An unmarked point invites the reading the marking exists to prevent — a measurement of class-based routing taken for a rival policy. The chart's four crowded points are placed by region rather than by nudged offsets, because three labels wanting the same 200 pixels is what an unplaced legend looks like.
